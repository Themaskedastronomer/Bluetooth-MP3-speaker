 ...
 *
 * v2.0 changes:
 *   - Album art: loads songname.jpg sidecar from SD via JPEGDEC
 *   - DISP button in player: short=toggle display, long=back to list
 *   - Idle I2S mute: silence output when not playing
 *   - DMA buffer tuning for reduced crackling
 *   - Soft fade in/out on play/pause/skip
 *   - Bass boost (+2dB) via audio.setTone()
 *   - BT heap fix: proper cleanup between sessions
 */
...
// ── Idle I2S Mute ─────────────────────────────────────────────────────────────
// Called when audio stops to silence I2S output (kills idle noise)
void muteI2S(){
  // Write a small buffer of zeros to flush/silence the I2S DMA
  int16_t silence[64] = {0};
  // audio library handles I2S internally; set volume to 0 as mute
  audio.setVolume(0);
}
...
void initMP3(){
  ...
  audio.setVolume(vol);
  // Bass boost +2, mid flat, treble flat — remove if distortion on PAM8406
  audio.setTone(0, 0, 0);
  if(!SD.begin(SD_CS)){
    tft.fillScreen(BLACK); hdr("MP3 MODE");
...
void setup(){
  ...
  tft.setTextColor(DIMGRAY); tft.setTextSize(1);
  tft.setCursor(95,155); tft.print("v2.0");
